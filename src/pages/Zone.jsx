...
function Zone() {
  ...
  const calculateZone = () => {
    if (!age || !heartRate) {
      alert("Enter your age and average heart rate");
      return;
    }
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 space-y-8 bg-[#f9f9f9] dark:bg-[#111] text-[#111] dark:text-white min-h-screen max-w-3xl mx-auto">
      <h1 className="text-3xl sm:text-4xl font-bold text-center">Zone Calculator</h1>
...
>
  Calculation 
          ...
          {zone && (
            <div className="text-center text-xl font-semibold mt-6">
             Result: <span className="text-[#111] dark:text-white">{zone}</span>
            </div>
          )}
          ...
          {zoneRanges && (
            <div className="mt-6 text-sm text-gray-600 dark:text-gray-400">
              <p className="font-semibold mb-2">HR Range by Zone (bpm):</p>
  ...
  {["all", "week", "month"].map((f) => (
    <button
      ...
      `}
    >
      {f === "all" ? "all" : f === "week" ? "weekly" : "monthly"}
    </button>
  ))}
</div>
        ...
        {filteredRecords.length > 0 && (
          <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-700 rounded-md p-6 shadow-sm">
            <h2 className="text-lg font-semibold mb-4">Zone Record</h2>
  ...
  {r.heartRate}bpm&nbsp;&nbsp;→&nbsp;&nbsp;{r.zone}
            ...
            {filteredRecords.length > 1 && (
              <div className="mt-8">
                <h2 className="text-lg font-semibold mb-4">HR Trends</h2>
